...
check("crea le dieci squadre", esito.teams === 10, `${esito.teams}`);
check("carica il listone intero", esito.players === 531, `${esito.players}`);
check("forma le rose", esito.contracts === 250, `${esito.contracts}`);
check("restituisce undici credenziali", esito.credentials?.length === 11, `${esito.credentials?.length}`);
...
check("nel database ci sono solo le impronte", impronte === "11" && !sql(`select string_agg("passwordHash", '') from "User";`).includes(password[0]));
...
const manager = esito.credentials.find((c) => c.role === "MANAGER");
...
const paginaManager = await entra(manager);
...
// AUTH_SECRET viene generata e conservata, e da lì in poi resta quella.
