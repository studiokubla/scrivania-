...
const sql = (q) =>
  execSync(`PGPASSWORD=maraka psql -h 127.0.0.1 -U maraka -d marakalegue -t -A -c "${q.replace(/"/g, '\\"')}"`)
    .toString()
    .trim();
...
await boss.waitForSelector(".avviso-ok", { timeout: 20000 });
...
await first.waitForSelector("#offerta", { timeout: 20000 });
...
async function offri(page, importo) {
  await page.reload();
  await page.waitForSelector("#offerta", { timeout: 20000 });
  ...
  await page.waitForSelector(".avviso-ok, .avviso-errore", { timeout: 20000 });
...
await third.waitForSelector('button:has-text("Non mi interessa")', { timeout: 20000 });
await third.click('button:has-text("Non mi interessa")');
await third.waitForSelector(".avviso-ok", { timeout: 20000 });
