...
const PSQL = `PGPASSWORD=maraka psql -h 127.0.0.1 -U maraka -d marakalegue -t -A`;
...
const m1 = await loginAs('manager1@marakalegue.it');
...
const m2 = await loginAs('manager2@marakalegue.it');
...
const used2 = sql(`select coalesce(max(used),0) from "OptionUsage" where type='FREE_AGENCY_OFFER' and "teamId" <> (select "teamId" from "User" where email='manager1@marakalegue.it');`);
...
const expectedWinner = sql(`select t.name from "Team" t join "User" u on u."teamId"=t.id where u.email='manager2@marakalegue.it';`);
